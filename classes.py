...
#   str g_line
    ...
    def extract_coordinate(self, coordinates_string, index, increment):
        coordinates_substring = coordinates_string[index[0] : index[0] + \
        increment]
    ...
    def extract_q_line_info(self, latitude, longitude, radius):
        ...
        latitude.set_degrees(self.extract_coordinate(coordinates_string, \
        index, 2))
        latitude.set_minutes(self.extract_coordinate(coordinates_string, \
        index, 2))
        latitude.set_direction(self.extract_direction(coordinates_string, \
        index))
        longitude.set_degrees(self.extract_coordinate(coordinates_string, \
        index, 3))
        longitude.set_minutes(self.extract_coordinate(coordinates_string, \
        index, 2))
        longitude.set_direction(self.extract_direction(coordinates_string, \
        index))
        radius[0] = self.extract_radius(coordinates_string, index)

    def extract_f_line_info(self, lower_limit, lower_height_limit):
        if self.f_line.find('SFC') != -1 or self.f_line.find('GND') != -1 or \
        self.f_line.find('GRD') != -1:
            lower_limit[0] = 0
        ...
        while alpha1 <= math.radians(360):
            ...
            * u2 ) ) )
            ...
            while math.fabs( sigma - sigma_p ) > 1e-12:
                ...
                delta_sigma = B * sin_sigma * ( cos_sigma_m_2 + ( 1 / 4 ) * B \
                * ( cos_sigma * ( -1 + 2 * cos_sigma_m_2 ** 2 ) - ( 1 / 6 ) * \
                B * cos_sigma_m_2 * ( -3 + 4 * sin_sigma ** 2 ) * ( -3 + 4 * \
                cos_sigma_m_2 ** 2 ) ) )
                sigma_p = sigma
            ...
            sin_sigma = math.sin(sigma)
            cos_sigma = math.cos(sigma)
            phi2 = math.atan2( sin_U1 * cos_sigma + cos_U1 * sin_sigma * \
            cos_alpha1, ( 1 - f ) * math.sqrt( sin_alpha ** 2 + ( sin_U1 * \
            sin_sigma - cos_U1 * cos_sigma * cos_alpha1 ) ** 2 ) )
            phi2 = math.degrees(phi2)
            lambda0 = math.atan2( sin_sigma * sin_alpha1, cos_U1 * cos_sigma \
            - sin_U1 * sin_sigma * cos_alpha1 )
            C = ( f / 16 ) * cos2_alpha * ( 4 + f * ( 4 - 3 * cos2_alpha ) )
            L = lambda0 - ( 1 - C ) * f * sin_alpha * ( sigma + C * sin_alpha \
            * ( cos_sigma_m_2 + C * cos_sigma * ( -1 + 2 * cos_sigma_m_2 ** 2 \
            ) ) )
            lambda2 = lambda1 + math.degrees(L)
            kml_file.print_normal("%s,%s,%s" % (lambda2, phi2, \
            int(round(self.get_altitude()))))
            alpha1 += angle
...
#   int indent_width
    ...
    def create_kml_source(self, document, poly_style, placemark, latitude, \
    longitude, look_at, point_polygon, deg_dec):
        ...
        self.print_normal('<visibility>%s</visibility>' % \
        document.get_visibility(self))
        ...
        self.print_normal('<outline>%s</outline>' % \
        poly_style.get_outline(self))
        self.unindent('</PolyStyle>')
        self.unindent('</Style>')
        self.print_normal('<Placemark>')
        self.indent('<name>%s</name>' % placemark.get_name())
        self.print_normal('<visibility>%s</visibility>' % \
        placemark.get_visibility(self))
        self.print_normal('<description>')
        self.indent(placemark.get_description())
        self.print_normal('')
        self.print_normal('latitude: %s%s%s (%s)' % (latitude.get_degrees(), \
        latitude.get_minutes(), latitude.get_direction(), \
        deg_dec.get_latitude()))
        self.print_normal('longitude: %s%s%s (%s)' % \
        (longitude.get_degrees(), longitude.get_minutes(), \
        longitude.get_direction(), deg_dec.get_longitude()))
        ...
        self.print_normal('<altitude>%s</altitude>' % \
        int(round(look_at.get_altitude())))
        ...
        self.print_normal('<altitudeMode>%s</altitudeMode>' % \
        int(round(look_at.get_altitude())))
        ...
        self.print_normal('<coordinates>%s,%s,%s</coordinates>' % \
        (point_polygon.get_longitude(), point_polygon.get_latitude(), \
        int(round(point_polygon.get_altitude()))))
        self.unindent('</Point>')
