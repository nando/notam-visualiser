...
class NotamProcessor(wx.Frame):
    ...
    def __init__(self):
        ...
        self.richTextCtrlSource = wx.richtext.RichTextCtrl(self, wx.ID_ANY, \
        ...
        self.Bind(wx.richtext.EVT_RICHTEXT_CONTENT_INSERTED, self.OnPaste, \
        self.richTextCtrlSource)
        self.Bind(wx.richtext.EVT_RICHTEXT_CONTENT_DELETED, self.OnPaste, \
        self.richTextCtrlSource)
    ...
    def OnHelp(self, event):
        stringMessage = 'Copy and paste the entire HTML source of the web ' + \
        'page containing the NOTAMs onto the text field.\n\nThis program ' + \
        'only works with Mozilla Firefox.'
        messageDialog = wx.MessageDialog(self, stringMessage, 'Help', \
    ...
    def OnOK(self, event):
        ...
        if singleChoiceList:
            singleChoiceDialog = wx.SingleChoiceDialog(self, \
            'Select a NOTAM to process:', 'NOTAM Selection', singleChoiceList)
            if os.name == 'posix':
            ...
            stringMessage = 'ERROR: Cannot read source. An incorrectly ' + \
            'formatted NOTAM is detected.'
...
class InformationDialog(wx.Frame):
    ...
    def __init__(self, informationList):
        ...
        title='Document and Placemark Information')
        
        staticTextDocumentName = wx.StaticText(self, wx.ID_ANY, \
        'Document name:')
        staticTextDocumentDescription = wx.StaticText(self, wx.ID_ANY, \
        'Document description:')
        staticTextPlacemarkName = wx.StaticText(self, wx.ID_ANY, \
        'Placemark name:')
        staticTextPlacemarkDescription = wx.StaticText(self, wx.ID_ANY, \
        'Placemark description:')
        
        size = wx.Size(self.GetSize().GetWidth() - \
        staticTextDocumentName.GetSize().GetWidth(), -1)
        
        self.textCtrlDocument = wx.TextCtrl(self, wx.ID_ANY, size=size)
        self.textCtrlPlacemark = wx.TextCtrl(self, wx.ID_ANY, size=size)
        self.textCtrlMultiDocument = wx.TextCtrl(self, wx.ID_ANY, \
        style=wx.TE_MULTILINE)
        self.textCtrlMultiPlacemark = wx.TextCtrl(self, wx.ID_ANY, \
        style=wx.TE_MULTILINE)
    ...
    def OnHelp(self, event):
        stringMessage = 'Enter the Document and Placemark names and ' + \
        'descriptions you want in the correct text fields.'
        messageDialog = wx.MessageDialog(self, stringMessage, 'Help', \
        wx.OK | wx.ICON_INFORMATION)
        messageDialog.ShowModal()
        messageDialog.Destroy()
    ...
    def OnOK(self, event):
        ...
        saveFileDialog = wx.FileDialog(None, 'Save File Dialog', 'C:\\', \
        'unnamed', '*.kml', \
        wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR)
        if saveFileDialog.ShowModal() == wx.ID_OK:
            if os.name == 'posix':
                fullPath = process.name(saveFileDialog.GetPath(), \
                saveFileDialog.GetWildcard())
            elif os.name == 'nt':
                fullPath = saveFileDialog.GetPath()
            saveFileDialog.Destroy()
            self.informationList.append(fullPath)
            process.generate(self.informationList)
            progressDialog = wx.ProgressDialog('Progress', \
            'Processing. Please wait...', 100, \
...
class ConfirmationDialog(wx.Dialog):
    ...
    def __init__(self, fullPath):
        ...
        staticTextConfirmation = wx.StaticText(self, wx.ID_ANY, \
        'Processing finished.')
    ...
    def OnHelp(self, event):
        stringMessage = 'Tick the correct checkboxes to indicate whether ' + \
        'you want to view the generated KML file and/or open it in ' + \
        'Google Earth.\n\nFor Windows users: If you choose both options, ' + \
        'then you must first close the KML view before the system can ' + \
        'run Google Earth.'
        messageDialog = wx.MessageDialog(self, stringMessage, 'Help', \
        wx.OK | wx.ICON_INFORMATION)
        messageDialog.ShowModal()
    ...
    def OnOK(self, event):
        ...
        if self.checkBoxRun.GetValue():
            if os.name == 'posix':
                ...
                filePath = 'C:\\Program Files\\Google\\Google Earth\\' + \
                'client\\googleearth.exe'
                commandPath = 'C:\\"Program Files"\Google\\"Google Earth"\\' + \
                'client\googleearth.exe'
                if os.path.exists(filePath):
                    os.system(commandPath + ' ' + \
                    process.normalize(self.fullPath))
                else:
                    stringMessage = 'Google Earth installation not found ' + \
                    '(C:\Program Files\...).\n\nThe program will now close.'
                    messageDialog = wx.MessageDialog(None, stringMessage, \
